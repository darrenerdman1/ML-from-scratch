...
class pReLU:
    ...
    def weightinit(self):
        if self.scaleweights:
            ...
            self.p=np.random.randn(1,self.nodes)
    ...
    def backprop(self,eta, Dnext):
        ...
        self.p -= eta*np.sum(Dnext*self.Act_derivativeP(self.p,self.H), axis=0)
        return D
    ...
    def momentum_backprop(self,eta, mu,Dnext):
        ...
        self.delp = mu*self.delp-eta*np.sum(Dnext*self.Act_derivativeP(selp.p,self.H), axis=0)
    ...
    def ada_backprop(self,eta,Dnext):
        ...
        self.p -= ada_etap*np.sum(Dnext*self.Act_derivativeP(self.p,self.H), axis=0)
        return D
    ...
    def RMS_prop(self,eta,gamma,Dnext):
        ...
        self.Gp = gamma*self.Gp +(1-gamma)*(np.sum(Dnext*self.Act_derivativeP(self.p,self.H), axis=0))**2
    ...
    def adamoptimizer(self,eta,mu,gamma,t,Dnext):
        ...
        self.mp = (mu*self.Gp +(1-mu)*(np.sum(Dnext*self.Act_derivativeP(self.p,self.H), axis=0)))/(1+mu**t)
        ...
        self.vp = (gamma*self.Gp +(1-gamma)*(np.sum(Dnext*self.Act_derivativeP(self.p,self.H), axis=0))**2)/(1+gamma**t)
