...
class MomentumNeuralNet:
    ...
    def weightinit(self):
        ...
        self.delw['W0']=np.zeros((self.indims,self.nodes[0]))
        self.biases['B0']=np.random.randn(1,self.nodes[0])
        self.delb['B0']=np.zeros((1,self.nodes[0]))
        for i in range(1,self.layers):
            ...
            self.delw['W'+str(i)]=np.zeros((self.nodes[i-1],self.nodes[i]))
            self.delb['B'+str(i)]=np.zeros((1,self.nodes[i]))
    ...
    def train(self,X,y,epochs,eta,mu,X_val=np.array([]),y_val=np.array([])):
        self.valerror=[]
        self.error=[]
        d={}
        self.Xmaxes=X.max(0)
        ...
        for i in range(epochs):
            ...
            for j in range(self.layers):
                self.delw['W'+str(j)]=mu*self.delw['W'+str(j)]-eta*self.Z['Z'+str(j)].T@d['d'+str(j)]
                self.weights['W'+str(j)]=self.weights['W'+str(j)]+self.delw['W'+str(j)]
                self.delb['B'+str(j)]=mu*self.delb['B'+str(j)]-eta*np.sum(d['d'+str(j)], axis=0)
                self.biases['B'+str(j)]=self.biases['B'+str(j)]+self.delb['B'+str(j)]
